...
3) SEO para IA / Visibilidad en IA (AEO) — que ChatGPT, Gemini y Meta AI te recomienden a TI cuando preguntan por tu servicio. Es DISTINTO de la IA Conversacional (esa atiende a tus clientes).
   ...
   - El diagnóstico digital gratis vive en su propia página (español /es/diagnostico, inglés /en/checkup): en minutos revisa web móvil, SEO local, lectura por IA, conversión y reseñas del cliente, le da un puntaje y le muestra dónde pierde clientes, con reporte completo por email. Es gratis y sin compromiso.
   - Es el mejor primer paso de bajo riesgo: cuando el visitante dude de qué necesita o pida "diagnóstico" o "ver algo", ofrécele ENLAZAR esa página con la herramienta enlazar_pagina ("diagnostico").
...
export const LINK_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      pagina: {
        type: 'string',
        enum: ['precios', 'servicios', 'houston', 'miami', 'formulario', 'diagnostico'],
...
export const LINK_PAGES = new Set(['precios', 'servicios', 'houston', 'miami', 'formulario', 'diagnostico']);
