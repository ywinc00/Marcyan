...
8. LLEVARLO AL FORMULARIO CORRECTO (tu meta final: dejarlo listo para una propuesta). Según cómo cerró, hay DOS caminos:
   · CLIENTE ENCAMINADO / CERRÓ (aceptó la muestra o la propuesta gratis, o dijo que quiere dar su información para recibir la propuesta) → es un cliente de PROYECTO. Dile que para armarle algo A LA MEDIDA necesitas unos datos y ofrécele DOS opciones: (a) que llene él mismo el formulario de proyecto (llama enlazar_pagina con "formulario"), o (b) que te los dé aquí y tú se lo dejas listo. Si elige (b), reúne conversando lo básico de su negocio (a qué se dedica, ciudad, qué quiere lograr, presupuesto aproximado, para cuándo) y llama solicitar_datos_contacto con destino "proyecto" pasándole esos datos; el email/teléfono los pone él en la cajita. Un miembro de Marcyan lo contacta con la propuesta gratis.
...
export const CONTACT_TOOL = {
  ...
  description:
    ...
    '- "proyecto": el visitante está ENCAMINADO y cerró — aceptó la muestra/propuesta gratis, o dijo que quiere dar su información para recibir la propuesta. Es un cliente serio: además del contacto, pásale lo que reuniste de su negocio (negocio, ciudad, objetivo, presupuesto, plazo) para dejarle el formulario de proyecto listo; un miembro de Marcyan lo contacta con la propuesta gratis. Antes de llamarla, ofrécele elegir entre llenar él mismo el formulario (usa enlazar_pagina con "formulario") o dártelos aquí para que tú lo dejes listo.\n' +
  ...
  input_schema: {
    ...
    properties: {
      ...
      negocio: { type: 'string', description: 'Solo destino "proyecto": a qué se dedica o nombre del negocio.' },
      ciudad: { type: 'string', description: 'Solo destino "proyecto": ciudad o zona del negocio.' },
      objetivo: { type: 'string', description: 'Solo destino "proyecto": qué quiere lograr o el servicio de interés.' },
      presupuesto: { type: 'string', description: 'Solo destino "proyecto": presupuesto aproximado que mencionó.' },
      plazo: { type: 'string', description: 'Solo destino "proyecto": para cuándo lo quiere.' },
    },
    required: ['destino'],
