...
export function analyzeSite({ html = '', https = false, hasSite = false, city = '', reviewsBucket = 'ns' } = {}) {
  ...
  const first30 = lc.slice(0, Math.floor(html.length * 0.3));
  ...
  const hasLocalBiz = hasJsonLd && /"@type"\s*:\s*["']?(LocalBusiness|Organization|ProfessionalService|Restaurant|Store|Dentist|Attorney|HomeAndConstructionBusiness)/i.test(html);
  ...
  checks.A4 = firstParagraph(html).length > 120 ? 'pass' : 'fail';

  // ── CONVERSIÓN ────────────────────────────────────────────────
  checks.C1 = /href=["']tel:/i.test(html) ? 'pass' : 'fail';
  checks.C2 = /(wa\.me|api\.whatsapp\.com)/i.test(html) ? 'pass' : 'fail';
  checks.C3 = /<form[\s>]/i.test(html) ? 'pass' : 'fail';
  const ctaEarly = /(href=["']tel:|wa\.me|api\.whatsapp\.com|<form[\s>]|mailto:|calendly|acuityscheduling|book(ing)?)/i.test(first30);
...
function repFromBucket(bucket, checks) {
  checks.R1 = bucket === '30+' ? 'pass' : bucket === '10-29' ? 'partial' : 'fail';
