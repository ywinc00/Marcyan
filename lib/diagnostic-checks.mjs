...
const FORM_EMBED_RE = /(typeform\.com|jotform|hsforms|hubspot|formspree|tally\.so|getform\.io|usebasin|forms\.gle|docs\.google\.com\/forms|calendly\.com|acuityscheduling|leadconnectorhq|gohighlevel|data-netlify|wufoo|cognitoforms)/i;
const hasFormSignal = (html) =>
  /<form[\s>]/i.test(html) ||
  /<input[^>]+type=["']?(email|tel)\b/i.test(html) ||
  FORM_EMBED_RE.test(html);
...
export function analyzeSite({ html = '', https = false, hasSite = false, city = '', reviewsBucket = 'ns' } = {}) {
  ...
  const hasLocalBiz = hasJsonLd && (
    /"@type"\s*:\s*["']?\w*(LocalBusiness|Organization|ProfessionalService|Restaurant|Store|Dentist|Attorney|Business|Company|Service|Contractor|Salon|Agent|Plumber|Electrician|Locksmith|RoofingContractor|AutoRepair|Physician|Clinic)/i.test(html) ||
    (/"telephone"\s*:/i.test(html) && /"address"\s*:/i.test(html))
  );
...
// Devuelve el href INTERNO de la página de contacto/cotización, o null.
export function findContactHref(html) {
  const hrefs = [...html.matchAll(/href\s*=\s*["']([^"']+)["']/gi)].map((m) => m[1].trim());
  const CONTACT_RE = /(contact|contacto|contactanos|contact-us|cotiza|cotizacion|quote|estimate|presupuesto|get-in-touch|book|agenda|schedule)/i;
  for (const h of hrefs) {
    if (!h || h.startsWith('#') || /^(mailto:|tel:|javascript:|data:)/i.test(h)) continue;
    if (/^https?:\/\//i.test(h) || h.startsWith('//')) continue; // solo internos (heurística conservadora)
    const path = h.split(/[?#]/)[0];
    if (CONTACT_RE.test(path)) return h;
  }
  return null;
}
