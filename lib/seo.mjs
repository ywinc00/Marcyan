...
// ── GSC: indexación (best-effort, NUNCA lanza) ────────────────
// Señal principal y fiable: la Sitemaps API agrega las URLs enviadas vs
// indexadas por sitemap. Opcionalmente enriquece con la URL Inspection API
// sobre la home para el estado de cobertura + último rastreo.
//   Devuelve { indexed, not_indexed, total, checked_at, issues, pages } o null.
//   Si no hay gsc_site_url, o la llamada 403/empty → null (degrada, no rompe).
export async function fetchIndexation(project) {
  const siteUrl = project && project.gsc_site_url;
  ...
  try {
    ...
    return null; // sin token (no configurado / WIF caído) → degradar
  }

  // 1) Sitemaps API → suma de submitted/indexed entre todos los sitemaps.
  let total = 0, indexed = 0;
  let hadSitemaps = false;
  const issues = [];
  try {
    ...
    if (res.ok) {
      ...
      for (const sm of sitemaps) {
        ...
        for (const c of contents) {
          total   += num0(c.submitted);
          indexed += num0(c.indexed);
        }
        const errs = num0(sm.errors);
        const warns = num0(sm.warnings);
        if (errs > 0) issues.push({ reason: `Errores en ${sm.path || 'sitemap'}`, count: errs });
        if (warns > 0) issues.push({ reason: `Avisos en ${sm.path || 'sitemap'}`, count: warns });
      }
    }
    // res no-ok (403/404/empty) → seguimos; quizá la inspección aún sirva.
  } catch (_) { /* degradar */ }

  // 2) URL Inspection sobre la home — opcional, enriquece pages[].
  const pages = [];
  try {
    const home = (project.domain && /^https?:/i.test(project.domain) ? project.domain
                : project.domain ? `https://${project.domain}`
                : (/^https?:/i.test(siteUrl) ? siteUrl : null));
    if (home) {
      const res = await fetch(GSC_INSPECT, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ inspectionUrl: home, siteUrl }),
      });
      if (res.ok) {
        const json = await res.json().catch(() => ({}));
        const idx = (json.inspectionResult && json.inspectionResult.indexStatusResult) || {};
        const verdict = idx.coverageState || idx.verdict || 'Desconocido';
        pages.push({
          url: home,
          state: verdict,
          last_crawl: idx.lastCrawlTime || null,
        });
        // Si la home no está indexada, lo reflejamos como issue legible.
        if (idx.verdict && idx.verdict !== 'PASS') {
          issues.push({ reason: `Home: ${verdict}`, count: 1 });
        }
      }
    }
  } catch (_) { /* la inspección es opcional */ }

  // Si no obtuvimos NADA (ni sitemaps ni inspección) → null.
  if (!hadSitemaps && pages.length === 0) return null;

  if (indexed > total) total = indexed; // defensa: nunca indexed > total
  return {
    indexed,
    not_indexed: Math.max(0, total - indexed),
    total,
...
// ════════════════════════════════════════════════════════════
//  Diagnóstico de acceso — ¿qué VE realmente la service account?
//  Si getAccessToken() funciona (token WIF OK) pero la API responde 403,
//  el problema NO es de infraestructura sino de PERMISOS/identidad de
//  propiedad. Estas funciones enumeran las propiedades a las que la SA SÍ
//  tiene acceso para detectar el desajuste típico:
//    · GSC: propiedad de DOMINIO ('sc-domain:x.com') vs PREFIJO de URL
//           ('https://www.x.com/') — son ACLs distintas; hay que consultar
//           exactamente la propiedad sobre la que la SA está autorizada.
//    · GA4: el número guardado no es el Property ID, o la SA no está añadida
//           a NIVEL DE PROPIEDAD.
//  Todo solo-lectura; no muta nada.
// ════════════════════════════════════════════════════════════
const GA4_ADMIN_BASE = 'https://analyticsadmin.googleapis.com/v1beta';

// GSC sites.list → [{ siteUrl, permissionLevel }]. siteUrl puede ser un
// prefijo ('https://www.x.com/') o un dominio ('sc-domain:x.com').
export async function gscListSites() {
  const token = await getAccessToken();
  const res = await fetch(`${GSC_BASE}/sites`, { headers: { Authorization: `Bearer ${token}` } });
  const json = await res.json().catch(() => ({}));
  if (!res.ok) {
    throw new Error(`GSC sites.list falló (${res.status}): ${(json.error && json.error.message) || 'error'}`);
  }
  return (json.siteEntry || []).map((s) => ({ siteUrl: s.siteUrl, permissionLevel: s.permissionLevel }));
}

// GA4 Admin accountSummaries → [{ account, accountName, property, propertyId, propertyName }].
// 'property' viene como 'properties/123456789'; extraemos el id numérico.
// Requiere la Analytics Admin API habilitada; si no, lanza (capturado arriba).
export async function ga4ListAccountSummaries() {
  const token = await getAccessToken();
  const res = await fetch(`${GA4_ADMIN_BASE}/accountSummaries?pageSize=200`, { headers: { Authorization: `Bearer ${token}` } });
  const json = await res.json().catch(() => ({}));
  if (!res.ok) {
    throw new Error(`GA4 accountSummaries falló (${res.status}): ${(json.error && json.error.message) || 'error'}`);
  }
  const out = [];
...
// Prueba puntual de acceso a UNA propiedad GA4 vía Data API metadata (ya
// habilitada). Devuelve { ok, status, error } sin lanzar.
export async function ga4ProbeProperty(propertyId) {
  if (!propertyId) return { ok: false, status: 0, error: 'sin property id' };
  const token = await getAccessToken();
  const res = await fetch(`${GA4_BASE}/properties/${encodeURIComponent(propertyId)}/metadata`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  const json = await res.json().catch(() => ({}));
  return { ok: res.ok, status: res.status, error: res.ok ? null : ((json.error && json.error.message) || 'error') };
}

// Prueba puntual de acceso a UNA propiedad GSC (searchAnalytics mínimo, 1 fila).
// Devuelve { ok, status, error } sin lanzar.
export async function gscProbeSite(siteUrl) {
  if (!siteUrl) return { ok: false, status: 0, error: 'sin site url' };
  const token = await getAccessToken();
  const range = defaultRange(7);
  const res = await fetch(`${GSC_BASE}/sites/${encodeURIComponent(siteUrl)}/searchAnalytics/query`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ startDate: range.startDate, endDate: range.endDate, dimensions: ['date'], rowLimit: 1 }),
  });
  const json = await res.json().catch(() => ({}));
  return { ok: res.ok, status: res.status, error: res.ok ? null : ((json.error && json.error.message) || 'error') };
}
...
export async function getMetrics(projectId, range) {
  ...
  `;
  const out = { gsc: [], ga4: [], indexation: null };
  for (const m of r.rows) {
    if (m.source === 'indexation') {
      // Snapshot único (no serie): exponemos el payload directamente, quedándonos
      // con el más reciente del rango. Forma:
      //   { indexed, not_indexed, total, checked_at, issues:[], pages:[] }
      const snap = { ...(m.payload || {}), fetched_at: m.fetched_at };
      if (!out.indexation || new Date(m.metric_date) >= new Date(out.indexation.metric_date || 0)) {
        out.indexation = { ...snap, metric_date: m.metric_date };
      }
      continue;
    }
...
export async function syncProject(project, days = 28) {
  ...
  const result = {
    ...
    indexation: { synced: 0, error: null },
  };

  if (!isConfigured()) {
    return {
      ...result, ok: false, error: 'not_configured',
      gsc: { synced: 0, error: 'not_configured' },
      ga4: { synced: 0, error: 'not_configured' },
      indexation: { synced: 0, error: 'not_configured' },
    };
  }
  ...
  // — Indexación (best-effort; un fallo NUNCA tumba la sync) —
  //   Guardamos un snapshot bajo source 'indexation' en metric_date = endDate.
  if (project.gsc_site_url) {
    try {
      const idx = await fetchIndexation(project);
      if (idx) {
        await upsertMetric(project.id, 'indexation', range.endDate, idx);
        result.indexation.synced = 1;
      }
    } catch (err) {
      // No marcamos result.ok=false: la indexación es complementaria.
      result.indexation.error = err.code === 'not_configured' ? 'not_configured' : (err.message || 'error');
    }
  }

  if (result.ok || result.gsc.synced || result.ga4.synced || result.indexation.synced) {
