...
// Flujo (cero secretos): VERCEL_OIDC_TOKEN (lo firma Vercel en runtime) →
// STS de Google lo canjea por un token federado → se impersona el SA para
// obtener un access_token con los scopes de GA4/GSC. No hay clave JSON que
// guardar ni rotar; respeta la política iam.disableServiceAccountKeyCreation.
async function fetchAccessToken() {
  const oidc = (process.env.VERCEL_OIDC_TOKEN || '').trim();
  if (!oidc) throw new NotConfiguredError('VERCEL_OIDC_TOKEN ausente (habilitá OIDC Federation en Vercel)');
